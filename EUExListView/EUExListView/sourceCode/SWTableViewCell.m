...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier containingTableView:(UITableView *)containingTableView leftUtilityButtons:(NSArray *)leftUtilityButtons rightUtilityButtons:(NSArray *)rightUtilityButtons {
    ...
    if (self) {
        ...
        //注册通知
        [[NSNotificationCenter defaultCenter]addObserver:self
                                                selector:@selector(responseNotificationLeft:)
                                                    name:@"scrollViewButtonViewLeft"
                                                  object:nil];
...
//实现通知的函数
- (void)responseNotificationLeft:(NSNotification*)notification{
...
- (void)SWSetBackgroundColor:(UIColor *)backgroundColor {
...
- (void)layoutSubviews {
    ...
    ///数据自适应
        ...
        CGFloat h = self.height-6;
        self.imageView.frame = CGRectMake(x, y, w, h);
        
        x = self.imageView.frame.origin.x+self.imageView.frame.size.width+8.0;
        y = self.imageView.frame.origin.y+8.0;
        ...
        w = self.detailTextLabel.frame.size.width;
        h = self.detailTextLabel.frame.size.height;
...
- (void)addUtilityButtonWithColor:(UIColor *)color title:(NSString *)title titleSize:(UIFont *)size bgColor:(UIColor *)bgColor {
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
